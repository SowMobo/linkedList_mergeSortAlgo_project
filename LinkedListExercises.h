...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  // check if the list is empty. So the head and tail will point to newNode
  if(!head_)
  {
    head_ = newNode;
    tail_ = newNode;
  ...
  {
      Node* cur = head_;
      // If the list is not empty, check if the newNode should be inserted as the head of the list.
      ...
      // Insert newNode somewhere in the list.
      else 
      {
        cur = cur->next;
        while(cur)
        {
            if(cur->data >= newNode->data)
            {
                Node *headCur = cur->prev;

                headCur->next = newNode;
                cur->prev = newNode;
                newNode->prev = headCur;
                newNode->next = cur;
                break;
            }
            cur = cur->next;
